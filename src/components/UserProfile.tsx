...
import { getOrdersByUser, createOrder, OrderItem } from '../integrations/supabase/orders';
...
const UserProfile: React.FC<UserProfileProps> = ({ userId }) => {
  ...
  const submitOrder = async () => {
    ...
    const newOrder = await createOrder(user.id, orderItems, total);
    if (newOrder) {
                ...
                order ? (
                  <div key={index} className="border rounded-md p-3">
                    ...
                    <div className="text-sm text-gray-500 mt-1">
                      ...
                      <p>Total: KSh {formatTotal(order.total)}</p>
                    </div>
                  </div>
                ) : null
              ))}
